...
const { groupResponse_Welcome, groupResponse_Remove } = require('./lib/well.js')
...
async function startnaimi() {
    const naimi = naimiConnect({
        ...
        browser: ['DH-BOT','Safari','1.0.0'],
        auth: state,
        version: getVersionWaweb() || [2, 2204, 13]
    })
    console.log(color('[DH DHIA]', 'cyan'), color(`NERO-YURRY-DHDHIA`, 'red'))
console.log(color('[DH DHIA]', 'cyan'), color('OWNER SEDANG ONLINE SEKARANG!', 'yellow'))

	console.log(color('[DH DHIA]', 'cyan'), color('SELAMAT DATANG OWNER ><', 'yellow'))
	
	console.log(color('======================', 'fuchsia'),color('\n[DH DHIA V1]', 'pink'),color('\nIni di buat oleh wibu...', 'yellow'),color('\n======================', 'fuchsia'))
	console.log(color('\nADA SARAN ATAU MASUKAN WA : +6285757562961', 'red'))
    store.bind(naimi.ev)
...
let docs = pickRandom(documents)
    naimi.ev.on('group-participants.update', async (update) => {
        if('add'){

            groupResponse_Welcome(naimi, update)
        }else{
            groupResponse_Remove(naimi, update)
        }    
            console.log(update)
    })
